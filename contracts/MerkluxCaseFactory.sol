 ...
 */
contract MerkluxCaseFactory is Secondary, MerkluxReducerRegistry {
    struct Case {
        address accuser;
        address merkluxCase;
        address store;
    ...
    constructor (string _version, uint256 _duration, bytes32 _merkluxCaseCode, bytes32 _storeCode) public Secondary() {
        version = _version;
        duration = _duration;
    ...
    function createCase(bytes32 _target) public {
        require(cases[_target].accuser == address(0));
        cases[_target].accuser = msg.sender;
    }

    function deployCase(bytes32 _caseHash, bytes _bytecode) public onlyAccuser(_caseHash) {
        // Should not be deployed before
        require(cases[_caseHash].merkluxCase == address(0));
        ...
        cases[_caseHash].merkluxCase = _deployContractWithByteCode(_bytecode);
    }
    ...
    function complete(bytes32 _caseHash) public onlyAccuser(_caseHash) {
        // Both contract should be deployed first
        address merkluxCase = cases[_caseHash].merkluxCase;
        address store = cases[_caseHash].store;
